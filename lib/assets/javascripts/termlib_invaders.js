...
TermlibInvaders = {
	...
	waveEnd: function() {
		var inv=this.env.invaders;
    if (inv.wave == 6) {
      var chars  = "lispnuwera2og:; ";
      var needed = "",
          symbols = [0, 11, 12, 1, 4, 13, 15, 3, 11, 6, 7, 8, 5, 2, 7, 8, 14, 15, 3, 9, 2, 2, 13, 15, 10, 3, 12, 0];

      for (var i=0; i < symbols.length; i++) {
        needed += chars[symbols[i]];
      }

      TermlibInvaders.writeToCenter.apply(this, [needed, TermlibInvaders.gameOverScreenWidth]);
			this.lock=false;
			this.handler=TermlibInvaders.splashScreenHandler;
			return;
    }
